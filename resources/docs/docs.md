...
The `home-routes` handler in the `bank-api.routes.home` namespace
...
The `home-page` function will in turn call the `bank-api.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `bank-api.handler` namespace:
...
Request middleware functions are located under the `bank-api.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `bank-api.dev-middleware` namespace found in
